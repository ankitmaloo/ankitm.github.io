...
If you start with Math based approach as OpenAI does, you can solve these kind of problems easily, but given the recipes not convincing to me if that would translate to a general purpose search that can work **equally** well on coding related chaining tasks which require more depth and less branching. So, these companies would likely train on all kinds of macros and maximally increase possible coverage. And, in that light, Noam's [post](https://x.com/polynoamial/status/2064210146558136827) about measuring models by how much token they use is certainly an attempt to point the discoure in this direction. 
