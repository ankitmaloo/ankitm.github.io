...
> **One-line summary:** Today, llms are trained in a multi step process post SFT.  RL -> Generate quality synthetic data → Self-Distillation on that → another round of RL (simplified). Fable-5 had a solution strategy constrained on how to compose code, and it struggled with a simpler solution until it exhausted all the greedy options. This is consistent with what a self-distillation recipe produces. 
...
If you start with Math based approach as OpenAI suggests, you can solve these kind of problems easily, but not super clear to me if that would translate to a general purpose search that can work **equally** well on coding related chaining tasks which require more depth and less branching. And, in that light, Noam's [post](https://x.com/polynoamial/status/2064210146558136827) about measuring models by how much token they use is certainly an attempt to point the discoure in this direction. 

---

#### Confidence Interval

I am about 90% confident[^7] as to this is a recipe. This [report](https://microsoft.ai/pdf/mai-thinking-1.pdf) from Microsoft is a great example of how companies go about things in RL stage. Then, this [report](https://openai.com/index/where-the-goblins-came-from/) from openai pretty much lays out the recipe for everyone to see. From OpenAI's goblin report, it shows they have a similar recipe too: ![from openai's goblin post](https://raw.githubusercontent.com/ankitmaloo/ankitm.github.io/refs/heads/main/_posts/openai_goblins.png)
