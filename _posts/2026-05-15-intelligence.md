...
A system optimizing for tight fit would not have flipped the frame. It would have stayed with Ptolemy and added the next epicycle. **Compression pressure[^10] on archival data would not surface a theory that is temporarily worse at predicting that data.**
...
Place LLMs against this and the question sharpens. They have the contingent features in abundance — symbolic manipulation, discrete categories, the appearance of central representation. Whether they have the convergent core, or only the mammal-shaped outputs of it, is an open question[^12]. The animal evidence does not settle that. What it does is establish that the question is well-posed: 
